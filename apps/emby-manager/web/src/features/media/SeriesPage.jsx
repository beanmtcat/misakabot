...
export default function SeriesPage({ notify, archived = false }) {
  ...
  async function saveSettings(event) {
    ...
    setSaving(true);
    try {
      const { id, name, system_path_mappings: _systemPathMappings, ...detail } = editing;
  ...
  return <section className="page-section media-section">
    ...
    {editing && <Dialog title={`设置 · ${editing.name}`} onClose={() => !saving && setEditing(null)}><form className="series-form" onSubmit={saveSettings}>
      ...
      <label className="path-map-field">手工路径映射<textarea value={editing.path_map_override || ''} onChange={(event) => setEditing({ ...editing, path_map_override: event.target.value })} placeholder="华语剧集/旧名称 (2026)/|/data/media/tv/华语剧集/新刮削名称 (2026)/" /><small>填写后覆盖该剧自动映射；清空则恢复自动规则。</small><span className="system-path-mapping"><b>系统规则路径映射</b>{editing.system_path_mappings?.length ? editing.system_path_mappings.map((mapping) => <code key={mapping}>{mapping}</code>) : <em>当前没有可用映射</em>}</span></label>
